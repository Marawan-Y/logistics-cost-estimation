# pages/3_KB_Bendix_Location_Info.py
...
def main():
    ...
    for i, loc in enumerate(locations):
        with st.expander(f"{loc['plant']} – {loc['country']}"):
            ...
            with col2:
                if st.button("Edit", key=f"edit_loc_{i}"):
                    st.session_state[f"edit_loc_{i}"] = True
                    st.rerun()
            with col3:
                if st.button("Delete", key=f"del_loc_{i}", type="secondary"):
    ...
    # ---------- Edit ----------
    for i, loc in enumerate(locations):
        if st.session_state.get(f"edit_loc_{i}", False):
            with st.form(f"edit_loc_form_{i}"):
                st.subheader(f"Edit Location: {loc['plant']}")
                col1, col2 = st.columns(2)
                with col1:
                    new_country = st.text_input(
                        "KB/Bendix – Country", value=loc["country"]
                    )
                with col2:
                ...
                with col1:
                    if st.form_submit_button("Update", type="primary"):
                        ...
                        if res["is_valid"]:
                            ...
                            st.session_state[f"edit_loc_{i}"] = False
                            st.rerun()
                        else:
                            for e in res["errors"]:
                                st.error(e)
