...
def main():
    ...
    for i, co2 in enumerate(co2_list):
        with st.expander(f"CO₂ Cost: €{co2['cost_per_ton']:.2f}/ton"):
    ...
    for i, co2 in enumerate(co2_list):
        if st.session_state.get(f"edit_co2_{i}", False):
            with st.form(f"edit_co2_form_{i}"):
                st.subheader(f"Edit CO₂ Cost: €{co2['cost_per_ton']:.2f}/ton")
